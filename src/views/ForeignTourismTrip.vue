<template>
    <header>
        <img :src="item.imgFo" alt="ForeignTourismFromTrip">
        <h4 class="itemTitle">{{item.nameTrForeign}}</h4>
    </header>
    <main class="details container pt-4">
        <div class="row">
            <div class="col p-5">
                <h5 class="titleDetails">details adventure</h5>
                <p class="text">
                    {{item.information}}
                </p>
...
export default {
    ...
    mounted() {
        fetch("http://localhost:3000/foreignTourism/" + this.id)
            .then(res => res.json())
